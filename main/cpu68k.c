...
uint8 *cpu68k_srambuff = NULL;
uint8 *cpu68k_sram = NULL;
unsigned int cpu68k_sramstart = 0;
unsigned int cpu68k_sramend = 0;
unsigned int cpu68k_sramlen = 0;
unsigned int cpu68k_sramactive = 0;
unsigned int cpu68k_sram_protect = 0;
...
unsigned int cpu68k_clocks_last1; // used in 6-button pad code
unsigned int cpu68k_clocks_last2;
...
t_ipclist *cpu68k_makeipclist(uint32 pc)
{
  ...
  do {
    ...
    if (instrs > size) {
      if (size > 10000)
	  //if (size > 100000) // DCGen does this, why??
...
{
  if(th1_timer) cpu68k_clocks_last1 = cpu68k_clocks;
  if(th2_timer) cpu68k_clocks_last2 = cpu68k_clocks;
