...
local int unzlocal_getByte (int fin, int *pi)	// Hiryu
{
    unsigned char   c;
    //int err = fread (&c, 1, 1, fin);
    int err = aioRead(fin, &c, 1);	// Sjeep
    if (err == 1)
    {
	*pi = (int) c;
	return UNZ_OK;
...
local int unzlocal_getShort (int fin, uLong * pX)	// Hiryu
{
    uLong           x;
    int             i;
    int             err;
...
local int unzlocal_getLong (int fin, uLong * pX)	// Hiryu
{
    uLong           x;
    int             i;
    int             err;
...
//    if (fseek (fin, 0, SEEK_END) != 0)
 //   uSizeFile = ftell (fin);
	...
	uSizeFile = aioLseek(fin, 0, SEEK_END);	// Sjeep
...
//	if (fseek (fin, uReadPos, SEEK_SET) != 0)
	if (aioLseek (fin, uReadPos, SEEK_SET) < 0)	// Sjeep
	    break;

//	if (fread (buf, (uInt) uReadSize, 1, fin) != 1)
	if (aioRead (fin, buf, (uInt) uReadSize) != uReadSize)	// Sjeep
	    break;
...
unzOpen (const char *path)
{
    ...
    //fin = fopen (path, "rb");
	fin = aioOpen (path, O_RDONLY); // Sjeep
    ...
    if (aioLseek (fin, central_pos, SEEK_SET) < 0)	// Sjeep
	err = UNZ_ERRNO;
...
//    if (fseek (s->file, s->pos_in_central_dir + s->byte_before_the_zipfile, SEEK_SET) != 0)
    if (aioLseek (s->file, s->pos_in_central_dir + s->byte_before_the_zipfile, SEEK_SET) < 0)	// Sjeep
	err = UNZ_ERRNO;
	...
	if ((file_info.size_filename > 0) && (fileNameBufferSize > 0))
	    ...
	    if (aioRead (s->file, szFileName, (uInt) uSizeRead) != uSizeRead)	// Sjeep
			err = UNZ_ERRNO;
	...
	if (lSeek != 0)
	{
	    ...
	    if (aioLseek (s->file, lSeek, SEEK_CUR) >= 0)	// Sjeep
			lSeek = 0;
	    else
		err = UNZ_ERRNO;
	}
	if ((file_info.size_file_extra > 0) && (extraFieldBufferSize > 0))
	    ...
	    if (aioRead (s->file, extraField, (uInt) uSizeRead) != uSizeRead)	// Sjeep
			err = UNZ_ERRNO;
	...
	if ((file_info.size_file_comment > 0) && (commentBufferSize > 0))
	    ...
	    if (aioRead (s->file, szComment, (uInt) uSizeRead) != uSizeRead)
			err = UNZ_ERRNO;
...
//	       s->byte_before_the_zipfile, SEEK_SET) != 0)
    if (aioLseek (s->file, s->cur_file_info_internal.offset_curfile +
	       s->byte_before_the_zipfile, SEEK_SET) < 0)		// Sjeep
		return UNZ_ERRNO;
...
/*		if (fseek (pfile_in_zip_read_info->file,
		...
		if (aioLseek (pfile_in_zip_read_info->file,
				pfile_in_zip_read_info->pos_in_zipfile +
				pfile_in_zip_read_info->byte_before_the_zipfile, SEEK_SET) < 0) // Sjeep
		    return UNZ_ERRNO;

		//if (fread (pfile_in_zip_read_info->read_buffer, uReadThis, 1,
		if (aioRead (pfile_in_zip_read_info->file,
					pfile_in_zip_read_info->read_buffer, uReadThis) != uReadThis)	// Sjeep
		    return UNZ_ERRNO;
...
/*    if (fseek (pfile_in_zip_read_info->file,
    ...
    if (aioLseek (pfile_in_zip_read_info->file,
	       pfile_in_zip_read_info->offset_local_extrafield +
	       pfile_in_zip_read_info->pos_local_extrafield, SEEK_SET) < 0)	// Sjeep
	return UNZ_ERRNO;

    //if (fread (buf, (uInt) size_to_read, 1, pfile_in_zip_read_info->file) != 1)
    if (aioRead (pfile_in_zip_read_info->file, buf, (uInt) size_to_read) != size_to_read) // Sjeep
		return UNZ_ERRNO;
...
//    if (fseek (s->file, s->central_pos + 22, SEEK_SET) != 0)
    if (aioLseek (s->file, s->central_pos + 22, SEEK_SET) < 0)	// Sjeep
		return UNZ_ERRNO;
	...
	*szComment = '\0';
	...
	if (aioRead (s->file, szComment, (uInt) uReadThis) != uReadThis)	// Sjeep
	    return UNZ_ERRNO;
    }
